// Complete Guinea administrative divisions data
// Based on the official structure: Regions > Prefectures > Sous-Prefectures (Communes/Quartiers)
...
// All 8 regions of Guinea with their prefectures and sous-prefectures
...
// Popular quartiers in Conakry for quick selection (featured)
...
// Helper function to get all quartiers as flat list for search
export function getAllQuartiers(): { region: string; prefecture: string; quartier: string }[] {
  const result: { region: string; prefecture: string; quartier: string }[] = [];

  for (const region of GUINEA_REGIONS) {
    for (const prefecture of region.prefectures) {
      for (const sp of prefecture.sousPrefectures) {
        result.push({
          region: region.nom,
...
export function getPrefecturesByRegion(regionCode: string): Prefecture[] {
  const region = GUINEA_REGIONS.find(r => r.code === regionCode);
  return region?.prefectures || [];
}

// Helper function to get sous-prefectures by prefecture
export function getSousPrefecturesByPrefecture(prefectureCode: string): SousPrefecture[] {
  for (const region of GUINEA_REGIONS) {
    const prefecture = region.prefectures.find(p => p.code === prefectureCode);
    if (prefecture) {
      return prefecture.sousPrefectures;
    }
  }
  return [];
}

// Default location for pre-selection
export const DEFAULT_LOCATION = {
  region: '01', // Conakry
  prefecture: 'CKY-01-01',
  quartier: 'Kipé',
};
